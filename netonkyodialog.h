...
*/
#ifndef NETONKYODIALOG_H
#define NETONKYODIALOG_H
...
namespace Ui {
class NetOnkyoDialog;
}

class NetOnkyoDialog : public QDialog, public ResponseListener
{
    Q_OBJECT
    
public:
    explicit NetOnkyoDialog(QWidget *parent, QSettings& settings, ReceiverInterface& Comm);
    ~NetOnkyoDialog();
...
private:
    struct DirectInput
    {
        ...
        DirectInput(int number, QString title)
        {
            this->number = number;
            this->title = title;
        }
    ...
    QString             m_TotalTime;
    QVector<DirectInput> m_DirectInputs;

    void moveEvent(QMoveEvent*event);
    void handleTime(QString str);
    void showAudioInfoLine();
    void requestListData(int pos);
